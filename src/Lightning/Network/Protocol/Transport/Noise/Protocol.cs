...
namespace Network.Protocol.Transport.Noise
{
   ...
   public sealed class Protocol
   {
      ...
      /// </summary>
      public const int MAX_MESSAGE_LENGTH = 65535 + Aead.TAG_SIZE;

      /// <summary>
      /// Minimum size of the protocol name in bytes.
      /// </summary>
      private static readonly int _minProtocolNameLength = "Noise_N_448_AESGCM_SHA256".Length;

      /// <summary>
      /// Maximum size of the protocol name in bytes.
      /// </summary>
      private const int MAX_PROTOCOL_NAME_LENGTH = 255;

      private static readonly Dictionary<string, HandshakePattern> _patterns = typeof(HandshakePattern).GetTypeInfo().DeclaredFields
         ...
         PatternModifiers modifiers = PatternModifiers.None)
      {
         ...
         HandshakePattern = handshakePattern;
         Cipher = cipher;
         Dh = DhFunction.Parse(curveName);
         Hash = hash;
         Modifiers = modifiers;

         versionPrefix.CopyTo(VersionPrefix);
      ...
      {
         if (s.Length < _minProtocolNameLength || s.Length > MAX_PROTOCOL_NAME_LENGTH)
         {
      ...
      private byte[] GetName()
      {
         ...
         Debug.Assert(protocolName.Length <= MAX_PROTOCOL_NAME_LENGTH);
      ...
      private ref struct StringSplitter
      {
         ...
         {
            int index = _s.IndexOf(_separator);

            if (index > 0)
            {
               var next = _s.Slice(0, index);
               _s = _s.Slice(index + 1);
