...
namespace Protocol.Test
{
   public class KeyDerivationTest
   {
      ...
      public void TestKeyDerivation()
      {
         ...
         PublicKey perCommitmentPoint = keyDerivation.DerivePublicKeyFromPrivateKey(perCommitmentSecret);
         PublicKey basePoint = keyDerivation.DerivePublicKeyFromPrivateKey(baseSecret);
         ...
         PublicKey pubkey2 = keyDerivation.DerivePublicKeyFromPrivateKey(privkey);
         ...
         pubkey2 = keyDerivation.DerivePublicKeyFromPrivateKey(privkey);
         Assert.Equal(pubkey.GetSpan().ToArray(), pubkey2.GetSpan().ToArray());
      }
   }
}